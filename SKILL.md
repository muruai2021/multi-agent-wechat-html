...
# multi-agent-wechat-html v3.1

> 公众号文章"写作 + 排版 + 验证"一体化工作流。
> v3.1 新增**默认写作偏好**（混合受众/技术为主、专业干货型、中篇 2500-3500 字、不含代码）。
...
## 默认写作偏好（v3.1 新增 · 用户未指定时一律按此走）

> 主编在 Step 1 出"主题三件套"之前，**默认采用以下偏好**，不必再向用户确认这四项；用户若有不同要求会主动说，否则按默认。

| 维度 | 默认值 | 含义 |
|------|--------|------|
| **目标读者** | 混合受众（技术为主） | AI 工程师 / Tech Lead / 架构师 / AI 产品经理 / 创业者为主体；非纯小白也非纯科研，技术语言可用但要"通俗技术博客"调性（阮一峰 / 酷壳 / 美团技术博客） |
| **风格调性** | 专业干货型 | 结构清晰、信息密集、有方法论 + 案例 + 数字；不走故事流、不走轻松科普流、不走犀利争议流 |
| **文章长度** | 中篇（2500-3500 字） | 含标题、章节标题、段落正文；超出 3500 字需用户确认；不足 2500 字补素材而非凑字 |
| **代码示例** | 不含代码，纯概念与场景 | 即使主题是技术性的，也只用文字描述工作流 / 模式 / 案例，不贴代码块、不贴命令；调研员/写手都按此约束 |

### 何时应当覆盖默认值（必须问用户）

- 用户在主题里写明了不同要求（如"我想写短文 1500 字" / "面向小白" / "用代码示例"）
- 主题本身**强烈不适配默认偏好**（如"教 React Hooks 用法"——这种主题必须有代码，主编应主动询问）
- 用户在 Step 1 反馈三件套时提出调整

### 主编出三件套时的隐性约束

主编在不问用户的前提下，按以下方式直接生成：

- **章节大纲字数分配** 按 3000 字目标做（正负 500 字浮动）
- **标题候选** 偏理性而非情绪化，避免营销标题党
- **开头钩子** 三种风格备选保留，但**默认推荐"现象切入型"**，符合技术读者阅读习惯
- **金句方向** 落在"工程动作"而非"哲学感叹"
- **调研员/写手** 自动收到"不含代码"约束
...
```
Step 1  主编出"主题三件套"
        ├─ 默认采用「写作偏好」（混合受众/技术为主、专业干货型、中篇 2500-3500 字、不含代码）
        ├─ 标题（2-3 个备选）
        ├─ 文章框架（章节大纲 + 字数分配）
...
| Agent | 模板 | 输出 | 职责 |
|-------|------|------|------|
| 主编（我） | 内置 | 主题三件套 | Step 1 **按默认写作偏好**出标题 / 框架 / 钩子（用户没明说就直接走默认），等待用户确认；Step 5 调排版模式；Step 6 调 verify.sh |
