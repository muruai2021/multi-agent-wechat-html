...
- 🆕 **默认写作偏好**：用户未指定时主编一律按以下默认值出三件套，无需反复询问
  - 目标读者：混合受众（技术为主）
  - 风格调性：专业干货型
  - 文章长度：中篇 2500-3500 字
  - 代码示例：不含代码，纯概念与场景
- 🆕 **覆盖默认值的判定逻辑**：用户明确指定 / 主题强烈不适配 / 用户在 Step 1 反馈三件套时调整
- 🆕 **主编隐性约束细化**：章节大纲按 3000 字目标分配、标题偏理性而非营销标题党、开头钩子默认推荐"现象切入型"、金句方向落在"工程动作"而非"哲学感叹"
- 🆕 **6 步工作流图** Step 1 加注"默认写作偏好"标记
- 🆕 **Agent 团队配置表** 主编职责加注"按默认写作偏好出三件套"

### 受益场景

- 用户输入 `/创作 [主题]` 时，主编不必每次询问受众/风格/长度/代码示例 4 项
- 调研员/写手自动收到"不含代码"约束
- 减少 Step 1 前的来回确认成本
